...
if __name__ == "__main__":
    ...
    if mode == "predict":
        ...
        3、如果想要获得预测框的坐标，可以进入retinanet.detect_image函数，在绘图部分读取top，left，bottom，right这四个值。
        4、如果想要利用预测框截取下目标，可以进入retinanet.detect_image函数，在绘图部分利用获取到的top，left，bottom，right这四个值
        在原图上利用矩阵的方式进行截取。
        5、如果想要在预测图上写额外的字，比如检测到的特定目标的数量，可以进入retinanet.detect_image函数，在绘图部分对predicted_class进行判断，
